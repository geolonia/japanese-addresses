...
const constructAddresses = (address, matchedData) => {
  ...
  if (match = zenkaku2hankaku(oazaChochomoku).match(/(.+)(\d+)([～・])(\d+)丁目/)) {
    ...
    for (let i = start; i <= end; i++) {
      patches.push({
        ...
        '大字町丁目名カナ': `${hiragana2katakana(oazaChochomokuKana)} ${i}`,
        '大字町丁目名ローマ字': `${r.romanize(hiragana2katakana(oazaChochomokuKana))} ${i}`,
        '小字・通称名': '',
        '緯度': null,
        '経度': null,
        '参照': 'TODO: 自治体のホームページのURLなど、リクエストの根拠となる情報を記載する。',
  ...
  // 「東京都国立市谷保（やほ）４丁目」のような住所
    ...
    patches.push({
      ...
      '大字町丁目名カナ': `${hiragana2katakana(oazaChochomokuKana)} ${match[2]}`,
      '大字町丁目名ローマ字': `${r.romanize(hiragana2katakana(oazaChochomokuKana))} ${match[2]}`,
      '小字・通称名': '',
      '緯度': null,
      '経度': null,
      '参照': 'TODO: 自治体のホームページのURLなど、リクエストの根拠となる情報を記載する。',
  ...
  // 「茨城県筑西市八丁台（はっちょうだい）」のような住所
    ...
    patches.push({
      ...
      '大字町丁目名カナ': hiragana2katakana(oazaChochomokuKana),
      '大字町丁目名ローマ字': r.romanize(hiragana2katakana(oazaChochomokuKana)),
      '小字・通称名': '',
      '緯度': null,
      '経度': null,
      '参照': 'TODO: 自治体のホームページのURLなど、リクエストの根拠となる情報を記載する。',
